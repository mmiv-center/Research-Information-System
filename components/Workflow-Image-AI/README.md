...
To simulate what the system does (testing) we 'trigger' the processing of a DICOM series by

```bash
ror trigger --keep
```

This call will create a new folder in the temp system folder (change with `ror config --temp_directory <new location>`). Inside that folder ror creates a copy of the selected image series (input / folder). With the '--keep' option the folder will stay around after processing - without the option the folder will be deleted. Any messages produced by the processing pipeline will end up in a 'log/' folder. Any output generated by the workflow should be placed in the 'output/' folder. Here is an example folder structure after processing.
